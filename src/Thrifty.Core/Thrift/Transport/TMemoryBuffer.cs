...
namespace Thrift.Transport {
    public class TMemoryBuffer : TTransport {
        ...
        public static T DeSerialize<T>(byte[] buf) where T : TAbstractBase {
            ...
            if (typeof (TBase).GetTypeInfo().IsAssignableFrom(typeof (T))) {
                var method = typeof (T).GetTypeInfo().GetMethod("Read", BindingFlags.Instance | BindingFlags.Public);
                var t = Activator.CreateInstance<T>();
                method.Invoke(t, new object[] {p});
                return t;
             } else {
                   var method = typeof (T).GetTypeInfo().GetMethod("Read", BindingFlags.Static | BindingFlags.Public);
